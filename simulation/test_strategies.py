...
class TestBaseAttackStrategy(unittest.TestCase):
  def test_get_expected_kept_damage_dice(self):
    ...
    self.assertEqual(2, strategy.get_expected_kept_damage_dice(attacker, target, 'attack', context, 0, 0))
    # spend 1 vp
    # skill roll params are 8k4, expected attack roll is 34, expected damage roll is 8k2
    self.assertEqual(2, strategy.get_expected_kept_damage_dice(attacker, target, 'attack', context, 0, 1))
    # spend 2 vp
    # skill roll params are 9k5, expected attack roll is 41, expected damage roll is 10k2
    self.assertEqual(2, strategy.get_expected_kept_damage_dice(attacker, target, 'attack', context, 0, 2))
    # spend 2 vp, 1 ap
    # skill roll params are 9k5+5, expected attack roll is 46, expected damage roll is 10k3
    self.assertEqual(3, strategy.get_expected_kept_damage_dice(attacker, target, 'attack', context, 1, 2))
    # spend 2 vp, 2 ap
    # skill roll params are 9k5+10, expected attack roll is 51, expected damage roll is 10k4
    self.assertEqual(4, strategy.get_expected_kept_damage_dice(attacker, target, 'attack', context, 2, 2))
    ...
    self.assertEqual(4, strategy.get_expected_kept_damage_dice(akodo, target, 'attack', context, 0, 0))
    # spend 1 vp, 0 ap
    # skill roll params are 10k7, expected attack roll is 53, expected damage roll is 10k6
    self.assertEqual(6, strategy.get_expected_kept_damage_dice(akodo, target, 'attack', context, 0, 1))
    # spend 0 vp, 1 ap
    # skill roll params are 10k5+5, expected attack roll is 49, expected damage roll is 10k5
    self.assertEqual(5, strategy.get_expected_kept_damage_dice(akodo, target, 'attack', context, 1, 0))
  ...
  def test_optimize_attack(self):
    ...
    attack = strategy.optimize_attack(weak_attacker, target, 'attack', context)
    ...
    attack = strategy.optimize_attack(strong_attacker, target, 'attack', context)
    self.assertFalse(attack is None)
    self.assertEqual(target, attack.target())
...
class TestKeepLightWoundsStrategy(unittest.TestCase):
  def test_keep_lw_small_hit(self):
    ...
    event = WoundCheckSucceededEvent(character, attacker, 10, 30)
  ...
  def test_keep_lw_near_defeat(self):
    ...
    context = EngineContext([Group([character]), Group([Character()])])
    strategy = KeepLightWoundsStrategy()
    event = WoundCheckSucceededEvent(character, attacker, 9001, 9002)
    responses = list(strategy.recommend(character, event, context))
  ...
  def test_take_sw(self):
    ...
    context.load_probability_data()
    strategy = KeepLightWoundsStrategy()
    event = WoundCheckSucceededEvent(character, attacker, 100, 101)
    responses = list(strategy.recommend(character, event, context))
...
class TestWoundCheckStrategy(unittest.TestCase):
  def test_declare_wound_check(self):
    ...
    event = LightWoundsDamageEvent(attacker, character, 10)
    responses = list(strategy.recommend(character, event, context))
    self.assertEqual(1, len(responses))
    response = responses[0]
