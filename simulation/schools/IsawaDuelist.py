...
class IsawaDuelist(Combatant):
    ...
    def r5t_post(self):
        if self.rank==5 and self.enemy.light==0 and self.enemy.serious > self.pre_sw:
            self.log("sets {0} back to 10 light wounds instead of 0", self.enemy.name)
            self.enemy.light = 10
            self.enemy.base_wc_threshold -= 10
    ...
    def make_attack(self):
        ...
        if self.rank >= 4 and self.attack_knack == "double_attack" and not success:
            if self.attack_roll >= self.enemy.tn - 20:
                self.log("R4T makes this a hit with no extra damage")
                self.attack_roll = 0
                success = True
        return success
